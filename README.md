...
Alternatively, the test repository can be cloned, and downloaded to the tester's machine. To make the testing step easier, there is a generateEpubs.sh script in the tests folder of repository that will generate an epub version of each test. To clone this repository to your machine, go to the "Code" menu at the top of this screen, select one of the local options to clone using git, or download the ZIP file for this repository to your machine.
...
Make a copy of the template file: [xx-template.json](https://w3c.github.io/epub-tests/reports/xx-template.json), change the file name to reflect the reading system and platform being tested (if multiple are being tested). 
...
If you have cloned the repository, you can submit test results by creating a new branch, uploading the files to the `results/` folder, and creating a PR containing your test results. 
